...
import InboxConversationCard, { type InboxConversationCardProps } from "../components/InboxConversationCard";

type ChatPreview = Pick<
  InboxConversationCardProps,
  "name" | "initials" | "avatarClassName" | "threadTitle" | "preview" | "timeLabel"
> & { id: string };
...
export default function InboxPage() {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(MOCK_CHATS[0]?.id ?? null);
    ...
    <div className="inline-flex min-h-0 w-full flex-1 flex-row justify-start items-stretch self-stretch overflow-hidden">
      ...
      <div className="inline-flex w-96 shrink-0 flex-col items-start justify-start self-stretch overflow-hidden border-r border-border bg-sidebar">
        <div className="flex w-96 flex-col items-start justify-start gap-3 border-b border-border p-5">
          <div className="justify-start font-['DM_Sans'] text-2xl font-bold text-foreground">Inkorg</div>
        ...
        <div className="flex min-h-0 w-96 flex-1 flex-col overflow-y-auto" role="list">
          {filtered.map((chat) => (
      ...
      {/* Höger: tom chattvy */}
      <section className="flex min-h-0 min-w-0 flex-1 flex-col bg-Colors-background" aria-label="Chatt">
