...
export function VideoChatLobby({ roomId: _roomId, userName, onConnect, onCancel }: Props) {
  ...
  const startStream = async (cameraId?: string) => {
    ...
    if (!isCameraOn) stream.getVideoTracks().forEach((t) => (t.enabled = false));
  };
  ...
  const loadDevices = async () => {
    ...
    const spks = devices
      ...
      .map((d, i) => ({ deviceId: d.deviceId, label: d.label || `Högtalare ${i + 1}` }));
    setCameras(cams);
    setSpeakers(spks);
  ...
  const handleSpeakerChange = (deviceId: string) => {
    ...
    const video = previewRef.current as any;
      ...
      <div className="self-stretch flex flex-col justify-start items-start gap-4 overflow-hidden">
        ...
        <div className="self-stretch inline-flex justify-start items-start gap-2">
          ...
          >
            ...
            <div className={`text-xs font-semibold font-['DM_Sans'] ${isMicOn ? "text-primary" : "text-destructive"}`}>
              Mikrofon
            </div>
          </div>
          ...
          >
            ...
            <div className={`text-xs font-semibold font-['DM_Sans'] ${isCameraOn ? "text-primary" : "text-destructive"}`}>
              Kamera
            </div>
          </div>
          ...
          >
            ...
            <div className={`text-xs font-semibold font-['DM_Sans'] ${showSettings ? "text-primary" : "text-foreground"}`}>
              Inställningar
            </div>
          </div>
        </div>
